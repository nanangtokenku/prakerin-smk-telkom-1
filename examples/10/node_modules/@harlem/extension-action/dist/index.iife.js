...
${codeFrame}` : message);
  ...
  // ../../packages/task/dist/index.bundler.mjs
  ...
  var Task = class extends Promise {
    constructor(executor, controller = new AbortController()) {
      ...
      super((_resolve, _reject) => {
        const resolve2 = (value) => execResolution(() => _resolve(value));
        ...
        executor(resolve2, reject, controller, onAbort);
      });
  ...
  // ../../packages/utilities/dist/index.bundler.mjs
  function identity(value) {
    return value;
  }
  function getType2(input) {
    return Object.prototype.toString.call(input).slice(8, -1).toLowerCase();
  }
  function isFunction2(value) {
    return getType2(value) === "function";
  }
  function isString2(value) {
    return getType2(value) === "string";
  }
  function normalise(matcher) {
    ...
    const patterns = [].concat(matcher).map(
      (pattern) => isString2(pattern) ? new RegExp(pattern) : pattern
    );
  ...
  function clone(value) {
    const input = (0, vue_exports.unref)(value);
    if (typeof input !== "object" || input === null) {
      return input;
    }
    const type = getType2(input);
    const cloner = CLONE_MAP[type] || CLONE_MAP.default;
    return cloner(input);
  }
  function isArray2(value) {
    return getType2(value) === "array";
  }
  function fromPath(value, path) {
    const nodes = isArray2(path) ? path : path.split("/");
  ...
  function setObjectValue(target, path, value) {
    const nodes = (isArray2(path) ? path : path.split("/")).slice();
    const key = nodes.pop();
    if (isNil(key) || key === "") {
  ...
  function isObject2(value) {
    return getType2(value) === "object";
  }
  function isMatchable(value) {
    return isObject2(value) && "include" in value && "exclude" in value;
  }

  // ../../core/dist/index.bundler.mjs
  ...
  function createInternalStore(name, initialState, eventBus, options) {
    ...
    const scope = (0, vue_exports.effectScope)();
    const stack2 = /* @__PURE__ */ new Set();
    let isSuppressing = false;
    const writeState = (0, vue_exports.reactive)(initialState);
    const readState = (0, vue_exports.readonly)(writeState);
    let resetSnapshot;
    function emit2(event, sender, data) {
    ...
    function track2(callback) {
      return scope.run(callback);
    }
    function hasRegistration(group, name2) {
      var _a22;
      return !!((_a22 = registrations[group]) == null ? void 0 : _a22.has(name2));
    }
    function getRegistration(group, name2) {
      var _a22;
      return (_a22 = registrations[group]) == null ? void 0 : _a22.get(name2);
    }
    ...
    function unregister(group, name2) {
      var _a22;
      (_a22 = registrations[group]) == null ? void 0 : _a22.delete(name2);
    }
    ...
    function getter(name2, getter2) {
      const output = track2(() => (0, vue_exports.computed)(() => getter2(readState)));
    ...
    function mutate(name2, sender, mutator, payload) {
      ...
      stack2.add(name2);
      let result;
      const trigger2 = (event) => emit2(event, sender, {
        name: name2,
        payload,
        result
      });
      trigger2(EVENTS.mutation.before);
      try {
        const producedState = (_a22 = producers.write(writeState)) != null ? _a22 : writeState;
        const producedPayload = (_b = producers.payload(payload)) != null ? _b : payload;
        result = mutator(producedState, producedPayload);
        trigger2(EVENTS.mutation.success);
      } catch (error) {
        trigger2(EVENTS.mutation.error);
        throw error;
      } finally {
        stack2.delete(name2);
        trigger2(EVENTS.mutation.after);
      }
      return result;
    }
    function mutation(name2, mutator) {
      const mutation2 = (payload) => {
        return mutate(name2, SENDER2, mutator, payload);
      };
      register("mutations", name2, () => mutation2);
      return mutation2;
    }
    function action(name2, body) {
      const mutate2 = (mutator) => write(name2, SENDER2, mutator);
      const action2 = async (payload) => {
        var _a22;
        let result;
        const trigger2 = (event) => emit2(event, SENDER2, {
          name: name2,
          payload,
          result
        });
        trigger2(EVENTS.action.before);
        try {
          const producedPayload = (_a22 = producers.payload(payload)) != null ? _a22 : payload;
          result = await body(producedPayload, mutate2);
          trigger2(EVENTS.action.success);
        } catch (error) {
          trigger2(EVENTS.action.error);
          throw error;
        } finally {
          trigger2(EVENTS.action.after);
        }
        return result;
      };
    ...
    function snapshot() {
      const snapshot2 = clone(initialState);
      const {
        value,
        getNodes,
        resetNodes
      } = traceObject();
      const apply = (branchAccessor = identity, mutationName = MUTATIONS.snapshot) => {
        write(mutationName, SENDER2, (state2) => {
          if (!snapshot2) {
          ...
          const source = fromPath(snapshot2, nodes);
          setObjectValue(state2, nodes, clone(source));
        });
      };
      return {
        apply,
    ...
    function reset(branchAccessor = identity) {
    ...
    const state = (_a2 = producers.read(readState)) != null ? _a2 : readState;
    return {
      name,
      allowsOverwrite,
      flags,
  ...
  function createInstance() {
    ...
    function emitCreated(store, state) {
      const created = () => {
        store.emit(EVENTS.ssr.initClient, SENDER2, state);
        store.emit(EVENTS.store.created, SENDER2, state);
        store.emit(EVENTS.ssr.initServer, SENDER2, state);
        store.emit(EVENTS.store.ready, SENDER2, state);
        store.emit(EVENTS.devtools.update, SENDER2, state);
      };
      if (installed) {
        return created();
      }
    ...
    function createStore2(name, state, options) {
      ...
      const destroy = () => {
        stores.delete(name);
        store.destroy();
        store.emit(EVENTS.store.destroyed, SENDER2, state);
        store.emit(EVENTS.devtools.update, SENDER2, state);
      };
      const getTrigger = (eventName) => {
        return (matcher, handler) => {
          const filter = getFilter(
            isMatchable(matcher) ? matcher : {
              include: matcher
            }
          );
  ...
  // src/index.ts
  var ABORT_STRATEGY = {
    error: (name, id, resolve2, reject, reason) => {
      reject(new ActionAbortError(name, id, reason));
    },
    warn: (name, id, resolve2, reject, reason) => {
  ...
  function actionExtension(options) {
    ...
    return (store) => {
      ...
      const actionState = (0, vue_exports.reactive)(/* @__PURE__ */ new Map());
      ...
      function action(name, body, options2) {
        ...
        return (payload, controller) => {
          ...
          if (!concurrent || isFunction2(concurrent) && !concurrent(payload, Array.from(instances.values()))) {
            abortAction(name, "New instance started on non-concurrent action");
          }
          if (autoClearErrors) {
            errors.clear();
          }
          const task = new Task(async (resolve2, reject, controller2, onAbort) => {
            var _a2;
            const id = Symbol(name);
            const complete = () => (tasks.delete(task), instances.delete(id));
            const fail = (reason) => strategies.abort(name, id, resolve2, reject, reason);
            let result;
            const trigger2 = (event) => store.emit(event, SENDER, {
              name,
              payload,
              result
            });
            ...
            trigger2(EVENTS.action.before);
            try {
              const producedPayload = (_a2 = store.producers.payload(payload)) != null ? _a2 : payload;
      ...
      function whenActionIdle(name, predicate, controller) {
        return new Task((resolve2, reject, controller2, onAbort) => {
          const isComplete = () => !isActionRunning(name, predicate);
          if (isComplete()) {
            return resolve2();
          }
          const unwatch = (0, vue_exports.watchEffect)(() => {
            if (isComplete()) {
              unwatch();
              resolve2();
            }
          });
          onAbort(() => (unwatch(), resolve2()));
        }, controller);
      }
      ...
      function suppressAbortError(action2) {
        return (payload, controller) => {
          return new Task(async (resolve2, reject, controller2, onAbort) => {
            onAbort(() => resolve2());
            try {
              const result = await action2(payload, controller2);
              resolve2(result);
            } catch (error) {
              isActionAbortError(error) ? resolve2() : reject(error);
            }
          }, controller);
        };
      }
