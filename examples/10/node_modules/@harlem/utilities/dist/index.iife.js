...
(() => {
  ...
  var PatchFlagNames, slotFlagsText, GLOBALS_WHITE_LISTED, isGloballyWhitelisted, range, listDelimiterRE, propertyDelimiterRE, styleCommentRE, HTML_TAGS, SVG_TAGS, VOID_TAGS, isHTMLTag, isSVGTag, isVoidTag, specialBooleanAttrs, isSpecialBooleanAttr, isBooleanAttr, unsafeAttrCharRE, attrValidationCache, propsToAttrMap, isKnownHtmlAttr, isKnownSvgAttr, escapeRE, commentStripRE, toDisplayString, replacer, EMPTY_OBJ, EMPTY_ARR, NOOP, NO, onRE, isOn, isModelListener, extend, remove, hasOwnProperty, hasOwn, isArray, isMap, isSet, isDate, isFunction2, isString2, isSymbol, isObject, isPromise, objectToString, toTypeString, toRawType, isPlainObject, isIntegerKey, isReservedProp, isBuiltInDirective, cacheStringFunction, camelizeRE, camelize, hyphenateRE, hyphenate, capitalize, toHandlerKey, hasChanged, invokeArrayFns, def, toNumber, _globalThis, getGlobalThis, identRE;
  var init_shared_esm_bundler = __esm({
    "../../node_modules/@vue/shared/dist/shared.esm-bundler.js"() {
      ...
      commentStripRE = /^-?>|<!--|-->|--!>|<!-$/g;
      toDisplayString = (val) => {
        return isString2(val) ? val : val == null ? "" : isArray(val) || isObject(val) && (val.toString === objectToString || !isFunction2(val.toString)) ? JSON.stringify(val, replacer, 2) : String(val);
      };
      replacer = (_key, val) => {
  ...
  function createStructuralDirectiveTransform(name, fn) {
    const matches2 = isString2(name) ? (n) => n === name : (n) => name.test(n);
    return (node, context) => {
      if (node.type === 1) {
        const { props } = node;
...
`);
  ...
  function isText$1(n) {
    return isString2(n) || n.type === 4 || n.type === 2 || n.type === 5 || n.type === 8;
  }
  ...
  function baseCompile(template, options = {}) {
    ...
    const ast = isString2(template) ? baseParse(template, options) : template;
  ...
  function defineAsyncComponent(source) {
    ...
    const load = () => {
      ...
      return pendingRequest || (thisRequest = pendingRequest = loader().catch((err) => {
        ...
        if (process.env.NODE_ENV !== "production" && comp && !isObject(comp) && !isFunction2(comp)) {
...
If this is a native custom element, make sure to exclude it from component resolution via compilerOptions.isCustomElement.` : ``;
  ...
  function renderList(source, renderItem, cache, index) {
    ...
    if (isArray(source) || isString2(source)) {
  ...
  function applyOptions(instance) {
    ...
    if (computedOptions) {
      for (const key in computedOptions) {
        ...
        const set2 = !isFunction2(opt) && isFunction2(opt.set) ? opt.set.bind(publicThis) : process.env.NODE_ENV !== "production" ? () => {
  ...
  function normalizePropsOptions(comp, appContext, asMixin = false) {
    ...
    if (isArray(raw)) {
      for (let i = 0; i < raw.length; i++) {
        if (process.env.NODE_ENV !== "production" && !isString2(raw[i])) {
      ...
      for (const key in raw) {
        ...
        if (validatePropName(normalizedKey)) {
          const opt = raw[key];
          const prop = normalized[normalizedKey] = isArray(opt) || isFunction2(opt) ? { type: opt } : Object.assign({}, opt);
          if (prop) {
...
For more details, see https://link.vuejs.org/feature-flags.`);
  ...
  function _createVNode(type, props = null, children = null, patchFlag = 0, dynamicProps = null, isBlockNode = false) {
    ...
    const shapeFlag = isString2(type) ? 1 : isSuspense(type) ? 128 : isTeleport(type) ? 64 : isObject(type) ? 4 : isFunction2(type) ? 2 : 0;
