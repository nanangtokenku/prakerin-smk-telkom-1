...
${codeFrame}` : message);
  ...
  function useListeners(listeners) {
    const _listeners = [].concat(
      isFunction2(listeners) ? listeners() : listeners
    );
    (0, vue_exports.onUnmounted)(() => _listeners.forEach(({ dispose }) => dispose()));
  }
    ...
    return (store) => {
      ...
      function useState(accessor, mutationName) {
        ...
        const setter = (value2) => store.write(name, SENDER, (state) => {
          parent(state)[key] = isFunction2(value2) ? value2(getter()) : value2;
        });
        return [
          getter,
          setter
